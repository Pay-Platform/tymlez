...
export async function deployViaTerraform({
  ...
  const entries = [
    {
      key: 'meter_collection_functions_object',
      value: functionsObject,
    },
    {
      key: 'meter_collection_functions_git_sha',
      value: gitSha,
    },
    {
      key: 'meter_collection_functions_git_tag',
      value: gitTag,
    },
  ];
  ...
  const runResult = await runTerraform({
    workspaceId: workspace.id,
    headers: terraformApiHeaders,
    message: `Deploy from platform-meter-collection: ${gitSha}`,
  });
