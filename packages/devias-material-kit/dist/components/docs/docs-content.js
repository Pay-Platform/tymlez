...
const Link = (props) => {
    ...
    return (<a href={href} rel="nofollow noreferrer noopener" target="_blank" {...other}>
      {children}
    </a>);
};
Link.propTypes = {
...
const Code = (props) => {
    ...
    return (<SyntaxHighlighter language={language} style={dracula} {...other}>
      {value}
    </SyntaxHighlighter>);
};
Code.propTypes = {
...
export const DocsContent = (props) => {
    const { content } = props;
    return (<DocsContentRoot>
      <Markdown escapeHtml renderers={{
            link: Link,
            code: Code,
        }} source={content}/>
    </DocsContentRoot>);
