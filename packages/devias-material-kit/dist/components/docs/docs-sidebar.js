...
export const DocsSidebar = (props) => {
    ...
    const content = (<Scrollbar sx={{
      ...
      <Box sx={{
        ...
        {sections.map((section) => (<DocsSidebarSection key={section.title} path={router.asPath} sx={{
                '& + &': {
                    mt: 3,
                },
            }} {...section}/>))}
    ...
    if (lgUp) {
        return (<Drawer anchor="left" open variant="permanent" PaperProps={{
                sx: {
    ...
    return (<Drawer anchor="right" onClose={onClose} open={open} variant="temporary" PaperProps={{
            sx: {
