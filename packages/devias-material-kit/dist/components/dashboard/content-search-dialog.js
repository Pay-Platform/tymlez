...
export const ContentSearchDialog = (props) => {
    ...
    return (<Dialog fullWidth maxWidth="sm" onClose={onClose} open={open} {...other}>
      <Box sx={{
            alignItems: 'center',
      ...
      <DialogContent>
        ...
        }} label="Search" onChange={(event) => setValue(event.target.value)} placeholder="Search..." sx={{ mt: 3 }} value={value}/>
        </form>
        {isLoading && (<Box sx={{
                display: 'flex',
        ...
        {showResults && (<>
            {Object.keys(results).map((type, index) => (<div key={index}>
                ...
                <Box sx={{
                  ...
                  {results[type].map((result, index) => (<Fragment key={result.title}>
                      <Box sx={{ p: 2 }}>
                        <Box sx={{
                        alignItems: 'center',
                        display: 'flex',
