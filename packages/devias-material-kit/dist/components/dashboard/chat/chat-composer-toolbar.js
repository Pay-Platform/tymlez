...
import { Avatar, Box, Chip, ClickAwayListener, InputAdornment, List, ListItem, ListItemAvatar, ListItemText, Paper, Popper, TextField, Typography, } from '@mui/material';
        ...
        <Scrollbar>
          <Box sx={{
            ...
            '& .MuiInputBase-root': {
              ...
              <TextField fullWidth onBlur={handleSearchBlur} onChange={handleSearchChange} onFocus={handleSearchFocus} placeholder="Search contacts" InputProps={{
            ...
            {recipients.reverse().map((recipient) => (<Chip avatar={<Avatar src={recipient.avatar}/>} key={recipient.id} label={recipient.name} onDelete={() => handleRemoveRecipient(recipient.id)} sx={{ mr: 2 }}/>))}
          </Box>
        </Scrollbar>
      </Box>
      {displayResults && (<ClickAwayListener onClickAway={handleSearchResultsClickAway}>
          <Popper anchorEl={containerRef?.current} open={isSearchFocused} placement="bottom-start">
            <Paper elevation={16} sx={{
              ...
              {filteredSearchResults.length === 0 ? (<Box sx={{
                    p: 2,
                    textAlign: 'center',
                }}>
                  ...
                  <List>
                    {filteredSearchResults.map((result) => (<ListItem button key={result.id} onClick={() => handleAddRecipient(result)}>
                        <ListItemAvatar>
                          <Avatar src={result.avatar}/>
                        </ListItemAvatar>
                        <ListItemText primary={result.name} primaryTypographyProps={{
                        noWrap: true,
                        variant: 'subtitle2',
                    }}/>
                      </ListItem>))}
