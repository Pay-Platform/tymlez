...
export const SocialPostCard = (props) => {
    ...
    return (<Card {...other}>
      <CardHeader avatar={<NextLink href="#" passHref>
            <Avatar component="a" src={authorAvatar}/>
          </NextLink>} disableTypography subheader={<Box sx={{
                alignItems: 'center',
                display: 'flex',
                mt: 1,
            }}>
      ...
      <Box sx={{
        ...
        <Divider sx={{ my: 3 }}/>
        {comments.map((comment) => (<SocialComment authorAvatar={comment.author.avatar} authorName={comment.author.name} createdAt={comment.createdAt} key={comment.id} message={comment.message}/>))}
        <Divider sx={{ my: 3 }}/>
        <SocialCommentAdd />
      </Box>
    </Card>);
