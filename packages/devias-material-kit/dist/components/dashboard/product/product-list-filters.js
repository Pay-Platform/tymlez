...
export const ProjectListFilters = (props) => {
    ...
    const handleDelete = (filterItem) => {
        setFilterItems((prevState) => prevState.filter((_filterItem) => {
            return !(filterItem.field === _filterItem.field &&
    ...
    return (<div {...other}>
      ...
      <Divider />
      {filterItems.length > 0 ? (<Box sx={{
                alignItems: 'center',
                display: 'flex',
                flexWrap: 'wrap',
                p: 2,
            }}>
          {filterItems.map((filterItem, i) => (<Chip key={i} label={<Box sx={{
                        alignItems: 'center',
                        display: 'flex',
                        '& span': {
                            fontWeight: 600,
                ...
                </Box>} onDelete={() => handleDelete(filterItem)} sx={{ m: 1 }} variant="outlined"/>))}
        </Box>) : (<Box sx={{ p: 3 }}>
      ...
      <Box sx={{
        ...
        <MultiSelect label="Category" onChange={(value) => handleCategoryChange(value)} options={categoryOptions} value={categoryValues}/>
        <MultiSelect label="Status" onChange={(value) => handleStatusChange(value)} options={statusOptions} value={statusValues}/>
        <MultiSelect label="Stock" onChange={(value) => handleStockChange(value)} options={stockOptions} value={stockValues}/>
      </Box>
    </div>);
};
