    ...
    switch (notification.type) {
        case 'job_add':
          ...
          <ListItemText primary={<Box sx={{
              ...
              </Box>} secondary={<Typography color="textSecondary" variant="caption">
...
export const NotificationsPopover = (props) => {
    ...
    return (<Popover anchorEl={anchorEl} anchorOrigin={{
            horizontal: 'left',
            vertical: 'bottom',
        }} onClose={onClose} open={open} PaperProps={{ sx: { width: 380 } }} transitionDuration={0} {...other}>
      <Box sx={{
            alignItems: 'center',
      ...
      {notifications.length === 0 ? (<Box sx={{ p: 2 }}>
          <Typography variant="subtitle2">
            There are no notifications
          </Typography>
        </Box>) : (<Scrollbar sx={{ maxHeight: 400 }}>
          <List disablePadding>
            {notifications.map((notification) => (<ListItem divider key={notification.id} sx={{
