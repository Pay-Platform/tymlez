...
export const CompanyOverview = (props) => {
    ...
    return (<div {...other}>
      ...
      <ImageList cols={3} gap={24} variant="masonry">
        {company.images.map((image) => (<ImageListItem key={image}>
      ...
      <Box sx={{ mt: 3 }}>
        <Grid container spacing={3}>
          {company.members.slice(0, 2).map((member) => (<Grid key={member.id} item sm={6} xs={12}>
              <Box sx={{
                borderColor: 'divider',
                borderRadius: 1,
                borderStyle: 'solid',
                ...
                mt: 1,
                  ...
                  {member.skillSet.map((skill) => (<Chip sx={{ m: 1 }} key={skill} label={skill}/>))}
                </Box>
              </Box>
            </Grid>))}
        </Grid>
