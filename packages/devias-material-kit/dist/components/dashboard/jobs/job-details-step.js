...
export const JobDetailsStep = (props) => {
    ...
    return (<div {...other}>
      ...
      <Box sx={{ mt: 3 }}>
        ...
        }} label="Tags" name="tags" onChange={(event) => setTag(event.target.value)} sx={{ mt: 3 }} value={tag}/>
        <Box sx={{ ml: -1 }}>
          {tagArray.map((_tag, i) => (<Chip onDelete={() => {
        ...
        <Box sx={{
          ...
          <MobileDatePicker label="Start Date" inputFormat="MM/dd/yyyy" value={startDate} onChange={handleStartDateChange} renderInput={(inputProps) => <TextField {...inputProps}/>}/>
          <Box sx={{ ml: 2 }}>
            <MobileDatePicker label="End Date" inputFormat="MM/dd/yyyy" value={endDate} onChange={handleEndDateChange} renderInput={(inputProps) => <TextField {...inputProps}/>}/>
          </Box>
        </Box>
      </Box>
      <Box sx={{ mt: 3 }}>
