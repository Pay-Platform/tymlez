        ...
        <CardContent>
          ...
          <Box sx={{ mt: 3 }}>
            <Grid container spacing={3}>
              {plans.map((plan) => (<Grid item key={plan.name} sm={4} xs={12}>
                  <Card elevation={0} onClick={() => setSelected(plan.name)} variant="outlined" sx={{
                cursor: 'pointer',
                ...(selected === plan.name && {
                ...
                justifyContent: 'space-between',
                        ...
                        {plan.current && (<Typography color="secondary.main" variant="caption">
                            Using now
                          </Typography>)}
                      </Box>
                    </CardContent>
