...
export const OverviewInbox = (props) => (<Card {...props}>
    <CardHeader title="Inbox"/>
    <Divider />
    <List disablePadding>
      {messages.map((message, index) => (<ListItem divider={index + 1 < messages.length} key={message.id}>
          <ListItemAvatar>
          ...
          <ListItemText disableTypography primary={<Box sx={{
              ...
              </Box>} secondary={<Typography color="textSecondary" variant="body2">
                {message.content}
              </Typography>} sx={{ pr: 2 }}/>
