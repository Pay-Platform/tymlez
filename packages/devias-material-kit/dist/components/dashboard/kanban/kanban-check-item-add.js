...
export const KanbanCheckItemAdd = (props) => {
    ...
    return (<Box sx={{ width: '100%' }} {...other}>
      {isExpanded ? (<Box sx={{
                alignItems: 'center',
                display: 'flex',
                width: '100%',
            }}>
          <OutlinedInput onChange={handleChange} placeholder="Add an item" value={name} sx={{
                flexGrow: 1,
                '& .MuiInputBase-input': {
                    px: 2,
                    py: 1,
        ...
        </Box>) : (<Button onClick={handleAdd} size="small" startIcon={<PlusIcon fontSize="small"/>}>
          Add Item
        </Button>)}
    </Box>);
};
