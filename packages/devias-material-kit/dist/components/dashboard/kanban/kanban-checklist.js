...
export const KanbanChecklist = (props) => {
    ...
    return (<KanbanChecklistRoot {...other}>
      <Card variant="outlined">
        <Box sx={{ p: 3 }}>
          <Box sx={{
            ...
            {editingName ? (<Box sx={{
                ...
                <OutlinedInput onChange={handleNameChange} value={name} sx={{
                flexGrow: 1,
                '& .MuiInputBase-input': {
                    px: 2,
                    py: 1,
                ...
                <Input disableUnderline fullWidth onClick={handleNameEdit} value={checklist.name} sx={{
        ...
        }} value={completePercentage} variant="determinate"/>
            </Box>
          </Box>
        </Box>
        <Divider />
        {checklist.checkItems.map((checkItem) => (<Fragment key={checkItem.id}>
            <KanbanCheckItem cardId={card.id} checkItem={checkItem} checklistId={checklist.id} editing={editingCheckItem === checkItem.id} onEditCancel={handleCheckItemEditCancel} onEditComplete={handleCheckItemEditComplete} onEditInit={() => handleCheckItemEditInit(checkItem.id)}/>
            <Divider />
          </Fragment>))}
        <CardActions>
