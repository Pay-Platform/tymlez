...
export const KanbanCardModal = (props) => {
    ...
    return (<Dialog fullWidth maxWidth="md" onClose={onClose} open={open} {...other}>
      <Grid container>
        <Grid item sm={8} xs={12}>
          <Box sx={{
            ...
            <TextField defaultValue={card.description} fullWidth label="Description" multiline onChange={(event) => handleDetailsUpdate({ description: event.target.value })} placeholder="Leave a message" rows={6} sx={{ mt: 3 }}/>
            {card.checklists.length > 0 && (<>
                <Typography sx={{ my: 3 }} variant="h6">
                  Checklist
                </Typography>
                <div>
                  {card.checklists.map((checklist) => (<KanbanChecklist card={card} checklist={checklist} key={checklist.id} sx={{ mb: 3 }}/>))}
                </div>
              </>)}
          </Box>
        </Grid>
        <Grid item xs={12} sm={4}>
          <Box sx={{
            ...
            <TextField fullWidth placeholder="Status" onChange={handleColumnChange} select SelectProps={{
            native: true,
        }} sx={{ mt: 2 }} value={card.columnId}>
              {Object.values(columns.byId).map((_column) => (<option key={_column.id} value={_column.id}>
                  {_column.name}
                </option>))}
            </TextField>
            <Box sx={{ mt: 2 }}>
            ...
            <Box sx={{ mt: 3 }}>
              <Typography color="textSecondary" component="h4" sx={{
            fontWeight: 600,
            mb: 2,
        }} variant="overline">
                Actions
              </Typography>
              {card.isSubscribed ? (<KanbanCardAction icon={<EyeOffIcon fontSize="small"/>} onClick={handleUnsubscribe}>
                  Unwatch
                </KanbanCardAction>) : (<KanbanCardAction icon={<EyeIcon fontSize="small"/>} onClick={handleSubscribe}>
                  Watch
                </KanbanCardAction>)}
            ...
            borderRadius: 1,
                ...
                <FormGroup>
                  {labels.map((label, index) => (<Fragment key={label}>
                      <FormControlLabel checked={card.labels.includes(label)} control={<Checkbox />} label={<Typography variant="body2">{label}</Typography>} onChange={handleLabelsChange} sx={{
                pl: 2,
                pr: 1,
                py: 0.5,
            }} value={label}/>
