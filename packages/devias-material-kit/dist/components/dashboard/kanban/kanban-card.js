...
export const KanbanCard = forwardRef((props, ref) => {
    ...
    return (<Box ref={ref} sx={{
        ...
        <Card onClick={handleOpen} raised={dragging} sx={{
            ...(dragging && {
          ...
          <Box sx={{ p: 3 }}>
            ...
            {card.labels.length > 0 && (<Box sx={{
                ...
                {card.labels.map((label) => (<Chip key={label} label={label} size="small" sx={{ m: 1 }}/>))}
              </Box>)}
            <Box sx={{
            alignItems: 'center',
            display: 'flex',
            ...
            '& svg:not(:first-of-type)': {
              ...
              <Box sx={{ flexGrow: 1 }}/>
              {card.members.length > 0 && (<AvatarGroup max={5}>
                  {card.members.map((member) => (<Avatar key={member.id} src={member.avatar}/>))}
                </AvatarGroup>)}
            </Box>
          </Box>
        </Card>
