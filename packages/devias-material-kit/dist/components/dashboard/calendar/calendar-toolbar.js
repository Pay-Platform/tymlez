...
export const CalendarToolbar = (props) => {
    ...
    return (<Box sx={{
      ...
      <Box sx={{
        ...
        <TextField label="View" name="view" onChange={handleViewChange} select size="small" value={view} sx={{
            ml: {
                xs: 'auto',
                md: 1,
            },
          ...
          {viewOptions.map((viewOption) => {
            ...
            return (<option key={viewOption.value} value={viewOption.value}>
                {viewOption.label}
              </option>);
        })}
        </TextField>
        <Button onClick={onAddClick} startIcon={<PlusIcon fontSize="small"/>} sx={{
            m: 1,
            order: {
                xs: -1,
                md: 0,
