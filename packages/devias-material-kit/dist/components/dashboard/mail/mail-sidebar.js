...
export const MailSidebar = (props) => {
    ...
    const content = (<div>
      ...
      <Box sx={{
        ...
        {Object.keys(groupedLabels).map((type) => (<Fragment key={type}>
            {type === 'custom' && (<ListSubheader disableSticky={true}>
                <Typography color="textSecondary" variant="overline">
                  Labels
                </Typography>
              </ListSubheader>)}
            <List disablePadding>
              {groupedLabels[type].map((label) => (<MailLabel active={currentLabel === label.id ||
                    (!currentLabel && label.id === 'inbox')} key={label.id} label={label} onClick={handleLabelClick}/>))}
            </List>
          </Fragment>))}
      </Box>
    </div>);
    if (mdUp) {
        return (<MailSidebarDesktop anchor="left" open={open} SlideProps={{ container: containerRef?.current }} variant="persistent" {...other}>
        {content}
      </MailSidebarDesktop>);
    }
    return (<MailSidebarMobile anchor="left" ModalProps={{ container: containerRef?.current }} onClose={onClose} open={open} SlideProps={{ container: containerRef?.current }} variant="temporary" {...other}>
      {content}
    </MailSidebarMobile>);
};
