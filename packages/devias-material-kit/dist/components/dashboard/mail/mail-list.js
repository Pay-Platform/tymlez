...
export const MailList = (props) => {
    ...
    return (<Box sx={{
      ...
      <Box sx={{
        ...
        }} placeholder="Search email" size="small" sx={{ width: 200 }}/>
      ...
      {emails.allIds.length === 0 ? (<Box sx={{
          ...
          <div>
            {emails.allIds.map((emailId) => (<MailItem email={emails.byId[emailId]} href={label && label !== 'inbox'
                    ? `/dashboard/mail?emailId=${emailId}&label=${label}`
                    : `/dashboard/mail?emailId=${emailId}`} key={emailId} onDeselect={() => handleDeselectOneEmail(emailId)} onSelect={() => handleSelectOneEmail(emailId)} selected={selectedEmails.includes(emailId)}/>))}
          </div>
        </>)}
    </Box>);
};
