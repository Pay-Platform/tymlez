...
const InvoiceRow = (props) => {
    const { invoice } = props;
    return (<TableRow key={invoice.id} sx={{
            boxShadow: 1,
    ...
    return (<div {...other}>
      <Scrollbar>
        <Table sx={{
          ...
          {group && (<TableBody>
              {Object.keys(groupedInvoices).map((status) => (<Fragment key={status}>
                  <TableRow>
                  ...
                  {groupedInvoices[status].map((invoice) => (<InvoiceRow invoice={invoice} key={invoice.id}/>))}
                </Fragment>))}
            </TableBody>)}
          {!group && (<TableBody>
      ...
      <TablePagination component="div" count={invoicesCount} onPageChange={onPageChange} onRowsPerPageChange={onRowsPerPageChange} page={page} rowsPerPage={rowsPerPage} rowsPerPageOptions={[5, 10, 25]}/>
    </div>);
};
