...
import { Box, Button, Card, Divider, FormControl, List, ListItem, ListItemAvatar, ListItemSecondaryAction, ListItemText, MenuItem, Select, TextField, Typography, } from '@mui/material';
export const CheckoutOrderSummary = (props) => {
    const { onQuantityChange, products, shippingTax, subtotal, total, ...other } = props;
    return (<Card variant="outlined" sx={{ p: 3 }} {...other}>
      <Typography variant="h6">Order Summary</Typography>
      <List sx={{ mt: 2 }}>
        {products.map((product) => (<ListItem disableGutters key={product.id}>
            ...
            <ListItemText primary={<Typography sx={{ fontWeight: 'fontWeightBold' }} variant="subtitle2">
                  {product.name}
                </Typography>} secondary={<Typography color="textSecondary" sx={{ mt: 1 }} variant="body1">
