...
import { Box, Button, Card, CardActions, CardContent, CardMedia, Divider, Grid, IconButton, ListItemIcon, ListItemText, Menu, MenuItem, Tooltip, Typography, } from '@mui/material';
...
export const GridList3 = () => {
    ...
    return (<Box sx={{
      ...
      <Grid container spacing={3}>
        {files.map((file) => (<Grid item key={file.id} md={4} xs={12}>
            <Card>
              {file.mimeType.includes('image/') ? (<CardMedia image={file.url} sx={{ height: 140 }}/>) : (<Box sx={{
                    alignItems: 'center',
              ...
              <Menu anchorEl={moreRef.current} anchorOrigin={{
                horizontal: 'left',
                vertical: 'top',
            }} elevation={1} onClose={handleMenuClose} open={openMenu} PaperProps={{
                sx: {
                    maxWidth: '100%',
                    width: 250,
                },
