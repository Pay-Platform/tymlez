...
export const GridList5 = () => (<Box sx={{
        backgroundColor: 'background.default',
        minHeight: '100%',
        p: 3,
    }}>
    <Grid container spacing={3}>
      {posts.map((post) => (<Grid item key={post.id} md={6} xs={12}>
          <Card>
            <CardHeader avatar={<Avatar src={post.author.avatar}/>} disableTypography subheader={<Box sx={{
                alignItems: 'center',
                display: 'flex',
                mt: 1,
            }}>
                ...
                </Box>} title={<Link color="textPrimary" variant="subtitle2">
                  {post.author.name}
                </Link>}/>
            <Box sx={{
            pb: 2,
