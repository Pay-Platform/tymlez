...
export const InputRadio = () => (<Box sx={{
    ...
    <RadioGroup sx={{
      ...
      {typeOptions.map((typeOption, index) => (<Paper key={typeOption.value} sx={{
            alignItems: 'flex-start',
            display: 'flex',
            padding: 2,
        }} variant="outlined">
          <FormControlLabel control={<Radio />} disabled={index === 2 && true} key={typeOption.value} label={<Box sx={{ ml: 2 }}>
                <Typography sx={{
