...
export const Modal4 = () => (<Box sx={{
    ...
    <Paper elevation={12} sx={{
      ...
      <Box sx={{ mt: 2 }}>
        <List disablePadding>
          {contacts.map((contact) => (<ListItem disableGutters key={contact.id}>
              <ListItemAvatar>
                <Avatar src={contact.avatar}/>
              </ListItemAvatar>
              <ListItemText disableTypography primary={<Link color="textPrimary" noWrap underline="none" variant="subtitle2">
                    {contact.name}
                  </Link>}/>
              {contact.isActive ? (<StatusIndicator size="small" status="online"/>) : (<Typography color="textSecondary" noWrap variant="caption">
