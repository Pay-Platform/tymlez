...
export const Form6 = () => {
    ...
    return (<Box sx={{
      ...
      <form onSubmit={(event) => event.preventDefault()}>
        ...
        <Box sx={{ mt: 2 }}>
          <DateTimePicker onChange={(newDate) => setStartDate(newDate)} label="Start date" renderInput={(inputProps) => (<TextField fullWidth {...inputProps}/>)} value={startDate}/>
        </Box>
        <Box sx={{ mt: 2 }}>
          <DateTimePicker onChange={(newDate) => setEndDate(newDate)} label="End date" renderInput={(inputProps) => (<TextField fullWidth {...inputProps}/>)} value={endDate}/>
        </Box>
        <Divider sx={{ my: 2 }}/>
        <Box sx={{
            alignItems: 'center',
