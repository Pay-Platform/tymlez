...
export const Form9 = () => (<Box sx={{
    ...
    <form onSubmit={(event) => event.preventDefault()}>
      <RadioGroup sx={{
        ...
        {typeOptions.map((typeOption) => (<Paper key={typeOption.value} sx={{
            alignItems: 'flex-start',
            display: 'flex',
            p: 2,
        }} variant="outlined">
            <FormControlLabel control={<Radio />} key={typeOption.value} label={<Box sx={{ ml: 2 }}>
