    ...
    return (<Box sx={{
      ...
      <form onSubmit={(event) => event.preventDefault()}>
        <div>
          ...
          <Box sx={{ mt: 2 }}>
            {['Full-Time'].map((tag) => (<Chip avatar={<Avatar>F</Avatar>} key={tag} label={tag} onDelete={() => { }} sx={{
                '& + &': {
                    ml: 1,
                },
            }} variant="outlined"/>))}
          ...
          <Box sx={{
            ...
            <Box sx={{ mr: 2 }}>
              <MobileDatePicker label="Start Date" onChange={(newDate) => setStartDate(newDate)} renderInput={(inputProps) => <TextField {...inputProps}/>} value={startDate}/>
            </Box>
            <MobileDatePicker label="End Date" onChange={(newDate) => setEndDate(newDate)} renderInput={(inputProps) => <TextField {...inputProps}/>} value={endDate}/>
          </Box>
        </div>
        <Box sx={{
            display: 'flex',
