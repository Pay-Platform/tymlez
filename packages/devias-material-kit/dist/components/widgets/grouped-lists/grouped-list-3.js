...
export const GroupedList3 = () => (<Box sx={{
    ...
    <Card>
      <CardHeader action={<IconButton>
          ...
          </IconButton>} title="Team Tasks"/>
      <Divider />
      <Scrollbar>
        <List sx={{ minWidth: 400 }}>
          {tasks.map((task, i) => (<ListItem key={task.id} divider={i < tasks.length - 1}>
              <ListItemText primary={<Link color="textPrimary" noWrap sx={{ cursor: 'pointer' }} variant="subtitle2">
                    {task.title}
                  </Link>} secondary={getDeadline(task)}/>
              <AvatarGroup max={3}>
                {task.members.map((member) => (<Tooltip key={member.name} title="View">
