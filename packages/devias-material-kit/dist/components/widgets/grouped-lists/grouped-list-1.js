...
export const GroupedList1 = () => (<Box sx={{
    ...
    <Container maxWidth="sm">
      <Card>
        ...
        <Divider />
        <List disablePadding>
          {activities.map((activity, i) => (<ListItem divider={i < activities.length - 1} key={activity.id}>
              <ListItemAvatar>
                <Avatar src={activity.customer.avatar} sx={{ cursor: 'pointer' }}/>
              </ListItemAvatar>
              <ListItemText disableTypography primary={<Link color="textPrimary" sx={{ cursor: 'pointer' }} underline="none" variant="subtitle2">
                    {activity.customer.name}
                  </Link>} secondary={<Typography color="textSecondary" variant="body2">
