...
export const GroupedList7 = () => (<Box sx={{
    ...
    <Card sx={{ maxWidth: 363 }}>
      ...
      <List disablePadding>
        {messages.map((message, index) => (<Box key={message.id}>
            <ListItem divider={index + 1 < messages.length}>
              <ListItemAvatar>
                <Avatar src={message.senderAvatar}/>
              </ListItemAvatar>
              <ListItemText disableTypography primary={index === 0 ? (<Badge color="primary" sx={{
                '.MuiBadge-badge': {
                    right: -16,
                    top: 11,
                },
                    ...
                    </Badge>) : (<Typography variant="subtitle2">
                      {message.senderName}
                    </Typography>)} secondary={<Box sx={{
                alignItems: 'center',
                display: 'flex',
