    ...
    return (<>
      ...
      <Box component="main" sx={{
        ...
        <Container maxWidth="lg">
          {sections.map((section) => (<Grid key={`section-${section.title}`} container spacing={3} sx={{
                mt: 0,
                pb: 8,
                '& + &': {
                    borderTop: 1,
              ...
              <Grid container item lg={9} spacing={3} xs={12}>
                {section.items.map((item) => (<Grid item key={`item-${item.title}`} md={4} sm={6} xs={12}>
                    <NextLink href={item.path} passHref>
                      <Card component="a" 
            // @ts-ignore
            target={item.newTab ? '_blank' : '_self'} sx={{
                    display: 'block',
                    textDecoration: 'none',
                    gridColumn: {
                        xs: 'span 3',
                    ...
                    display: 'flex',
                            ...
                            {item.newTab && (<ExternalLinkIcon sx={{
...
Browse.getLayout = (page) => (<MainLayout>
