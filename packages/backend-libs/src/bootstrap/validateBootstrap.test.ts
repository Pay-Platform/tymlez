...
describe('validateBootstrap', () => {
  ...
  it('should return validated bootstrap even if solar_details is missing', async () => {
    const mockedBootstrap = getMockedBootstrap();
    const bootstrapWithoutSolarDetail = produce(mockedBootstrap, (draft) => {
  ...
  it('should throw correct error if meter_details is missing', async () => {
    const mockedBootstrap = getMockedBootstrap();
    const bootstrapWithoutMeterDetail = produce(mockedBootstrap, (draft) => {
      //@ts-expect-error force error in test
  ...
  it('should throw correct error if meter has more than 6 channels', async () => {
    ...
    await expect(
      validateBootstrap({
        bootstrap: produce(mockedBootstrap, (draft) => {
      ...
      /channel_details.+ must contain less than or equal to 6 items/,
    );
  });
  ...
  it('should throw correct error if channel > circuit_name not one of the circuits in the meter', async () => {
    ...
    await expect(
      validateBootstrap({
        bootstrap: produce(mockedBootstrap, (draft) => {
          draft.site_details.main.meter_details[
            'meter-2'
          ].channel_details[0].circuit_name = 'invalid circuit';
        }),
      }),
  ...
  it('should throw correct error if solar.name not equal to its record key', async () => {
    ...
    await expect(
      validateBootstrap({
        bootstrap: produce(mockedBootstrap, (draft) => {
          assert(draft.site_details.main.solar_details);
  ...
  it.each([null, '', 'api key 1'])(
    'should throw correct error if api_key is "%s" (non-empty)',
    async (apiKey) => {
      const mockedBootstrap = getMockedBootstrap();
  ...
  it.each([null, '', 'password 1'])(
    'should throw correct error if password is "%s" (non-empty)',
    async (password) => {
      const mockedBootstrap = getMockedBootstrap();
  ...
  it.each([undefined, null, ''])(
    'should throw correct error if password is "%s" (empty)',
    async (password) => {
      const mockedBootstrap = getMockedBootstrap();
...
function getMockedBootstrap({
  addSecret,
}: { addSecret?: boolean } = {}): IBootstrap {
  return {
    client_detail: {
      name: 'cohort',
      label: 'Cohort',
