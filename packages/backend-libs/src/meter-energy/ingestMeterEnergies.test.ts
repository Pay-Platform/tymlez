...
import { ingestMeterEnergies } from './ingestMeterEnergies';
...
describe('ingestMeterEnergies', () => {
  ...
  `, () => {
    beforeAll(async () => {
      ...
      await ingestMeterEnergies(getMockIngestMeterEnergiesInput());
    });

    it('should call insertMeterEnergies once with correct information', () => {
      expect(insertMeterEnergies).toHaveBeenCalledTimes(1);
      expect((insertMeterEnergies as jest.Mock).mock.calls)
  ...
  describe(`
    ...
    let response: ReturnType<typeof ingestMeterEnergies>;

    beforeAll(() => {
      jest.clearAllMocks();

      response = ingestMeterEnergies(
        getMockIngestMeterEnergiesInput({
          energyResponses: [
            {
              meter_id: 'meter_id 1',
              duration: 300,
              ...
              eRealKwh: [],
            } as any as IWattwatchersLongEnergyResponseItem,
          ],
        }),
      );
    });
    ...
    it('should not call insertMeterEnergies', () => {
      expect(insertMeterEnergies).toHaveBeenCalledTimes(0);
    });
  });
});

function getMockIngestMeterEnergiesInput(
  overrides?: Partial<Parameters<typeof ingestMeterEnergies>[0]>,
): {
  requestId: string;
