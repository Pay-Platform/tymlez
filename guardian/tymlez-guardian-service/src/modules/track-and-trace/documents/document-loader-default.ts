...
export class DefaultDocumentLoader extends DocumentLoader {
  public async has(iri: string): Promise<boolean> {
    if ((didContexts.contexts as Map<string, any>).has(iri)) {
      return true;
    }
    if ((credentialsContexts.contexts as Map<string, any>).has(iri)) {
      return true;
    }
    if ((securityContexts.contexts as Map<string, any>).has(iri)) {
      return true;
    }
    return false;
  }
